$(function () {
    function format(d) {
        let html = `<div style="padding-left: 47px"><i class="fa fa-users"></i> Nhân viên liên hệ <a data-partner="${d.id}" href="javascript:void(0)" class="add-partner-contact" style="padding-left: 10px"><i class="fa fa-plus text-success"></i> Thêm liên hệ</a></div>
                        <table>
        ...
        var tr = "";
        for (let i = 0; i < d.partnerContacts.length; i++) {
            tr += `<tr> <td></td><td></td>
                            <td class="contactName_${d.partnerContacts[i].id}">${d.partnerContacts[i].contactName}</td>
        ...
        return html.replace("{{body}}", tr);
    }

    var partnerList = function (callback) {
        $.ajax({
            url: "/OneOperator/Partner/JsonData",
            type: "get",
            dataType: "json",
            success: (res) => {
                callback(res);
    ...
    partnerList(function (res) {
        ...
        var table = $("#example").DataTable({
            ...
            "columns": [
                ...
                { "data": "partnerName" },
                { "data": "groupPartner.groupPartnerName" },
                { "data": "address" },
                { "data": "telephone" },
                { "data": "email" },
                {
                    mRender: function (data, type, row) {
                        return `<a href="/OneOperator/Partner/Edit/${row.id}"><i class="fa fa-edit text-success"></i> Sửa</a>
                            | <a class="delete-item" href="javascript:void(0)" data-id="${row.id}">
